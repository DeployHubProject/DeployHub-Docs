...
| --- | --- |
| **DMScript in Database** | A _Function_ or _Procedure_ is written in DMScript and stored in the DeployHub Database (a "Stored Procedure"). |
| **Local Script** | A _Function_ or _Procedure_ is written in any Scripting Language that can be executed by the operating system on which the DeployHub Deployment Engine is installed. Executes locally to the Deployment Engine. |
...
|**Command Line Interpreter**| Your _Function_ or _Procedure_ will be written using a particular language such as Python.  This field in the fully qualified path and name of your interpreter.  DeployHub will need this to execute your process.|
...
|**Copy to Remote** | A checkbox that causes DeployHub to copy the script from the directory indicated by the filepath field on the deployment engine to the target _Endpoint_. The script is then executed there. |
