...
Package identity in DeployHub is based on a standardized **PURL** (Package URL) — see the [Glossary](../../../reference/glossary/) — so the same package is matched consistently whether it comes from a CVE record (OSV data) or an SBOM component (CycloneDX data).
