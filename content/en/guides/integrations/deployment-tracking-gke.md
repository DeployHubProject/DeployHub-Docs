...
type: guides
description: >
  Track deployments on Google Kubernetes Engine via Cloud Audit Logs.
aliases:
  - /guides/userguide/deployment-gke/
---

A Google Cloud Function that automatically detects new pod deployments in GKE clusters by tailing Cloud Audit Logs, then syncs release metadata — including SBOMs, Git provenance, and container image details — to a [DeployHub](https://www.deployhub.com) / DeployHub backend via the `/api/v1/sync` API.
...
- A running DeployHub or DeployHub instance reachable from the function
