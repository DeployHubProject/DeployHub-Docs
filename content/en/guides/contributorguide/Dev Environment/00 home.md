---
title: "Adventure with Abraham Ortelius"
linkTitle: "Adventure with Abraham Ortelius"
weight: 3
description: >
  The journey begins here.
---

# --- Adventure with Abraham Ortelius ---

## Motivation
- This is a navigational map in the spirit of [Abraham Ortelius](https://en.wikipedia.org/wiki/Abraham_DeployHub)
- Please read and explore the documentation for each subject
- Google is your friend for when you get lost
- To answer your question "Yes it is hard" but you can do it
- If Abraham Ortelius could draw maps in the 1500s without our modern day technology you can do this
- Abraham Ortelius was a DevOps Engineer and Microservices pioneer by containerising the world through cartography
- Please help us improve the cartography

## Organizations
- [CNCF](https://www.cncf.io/)
- [CNCF Cloud Native Landscape](https://landscape.cncf.io/)
- [DeployHub](https://www.deployhub.com/)

### What are we learning?
- Backend development in [Go](https://go.dev/)
- Frontend development with [Node.js](https://nodejs.org/) and [Next.js](https://nextjs.org/)
- Containerization with [Docker](https://www.docker.com/) or [Podman](https://podman.io/)
- [ArangoDB](https://www.arangodb.com/)
- [Kubernetes](https://kubernetes.io/)
- [Build pipelines](https://opensource.com/article/19/4/devops-pipeline)
- [DeployHub](https://www.deployhub.com/)
- [Cloud Computing](https://www.zdnet.com/article/what-is-cloud-computing-everything-you-need-to-know-about-the-cloud/)
- [Argo CD](https://argo-cd.readthedocs.io/en/stable/user-guide/parameters/)
- Software Supply Chain security and SBOMs

### What are we building?

#### DeployHub PDVD — Package, Dependency, Vulnerability Dashboard
DeployHub tracks SBOMs through the entire release lifecycle, mapping components to CVEs so teams can rapidly respond to supply chain threats.

- **Backend:** [ortelius/ortelius](https://github.com/ortelius/ortelius) — Go REST API (client/server for tracking SBOM → release → CVE)
- **Frontend:** [ortelius/frontend](https://github.com/ortelius/frontend) — Node.js + Next.js web UI

#### Event Driven Architecture with DeployHub
- [DeployHub Docs](https://github.com/DeployHubProject/DeployHub-Docs/blob/main/content/en/guides/userguide/integrations/DeployHubKeptn.md)

## Demo Apps to Deploy and Containerise

### Go Backend — [ortelius](https://github.com/ortelius/ortelius)
- `PLEASE READ the README`
- Fork the repo & clone
- [Go Documentation](https://go.dev/doc/)
- [Go Modules](https://go.dev/ref/mod)
- Run: `go run .`
- Test: `go test ./...`
- Build container: `docker build -t ortelius .`

### Node.js + Next.js Frontend — [frontend](https://github.com/ortelius/frontend)
- `PLEASE READ the README`
- Fork the repo & clone
- [Next.js Documentation](https://nextjs.org/docs)
- Install: `npm install`
- Run dev: `npm run dev`
- Build: `npm run build`

## DeployHub Jargon Glossary
- API
- [Argo CD](https://argo-cd.readthedocs.io/en/stable/user-guide/parameters/)
- Backend
- [Blast radius](https://www.deployhub.com/know-your-microservice-blast-radius/)
- Containers
- Continuous Integration
- Continuous Delivery
- CVE (Common Vulnerabilities and Exposures)
- Declarative
- Event driven
- Frontend
- [GitOps](https://about.gitlab.com/topics/gitops/)
- [Go](https://go.dev/)
- [Helm.sh](https://helm.sh/)
- [IaC](https://www.redhat.com/en/topics/automation/what-is-infrastructure-as-code-iac)
- Imperative
- [Kubernetes](https://kubernetes.io/docs/home/)
- [Markdown](https://www.markdownguide.org/)
- Microservices
- [Next.js](https://nextjs.org/)
- [Node.js](https://nodejs.org/)
- [Opensource](https://opensource.com/resources/what-open-source)
- Package Manager
- [ArangoDB](https://www.arangodb.com/)
- SBOM (Software Bill of Materials)
- [Terraform](https://www.terraform.io/)

## Learning Platforms
- [Udemy.com](https://udemy.com)
- [Kodekloud.com](https://kodekloud.com)
- [Go Tour](https://go.dev/tour/)
- [Next.js Learn](https://nextjs.org/learn)
- [101 Days of DevOps](https://www.101daysofdevops.com/courses/)

## Linux & Open Source
- [Itsfoss.com](https://itsfoss.com/)
- [Linuxhandbook.com](https://linuxhandbook.com/)
- [Bash](https://ss64.com/links/bash.html)
- [Bash Bible](https://github.com/dylanaraps/pure-bash-bible)
