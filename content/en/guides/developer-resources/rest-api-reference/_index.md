...
type: guides
description: >
  REST and GraphQL endpoints, Kafka event schema, and the database schema, for engineers integrating with DeployHub directly.
aliases:
  - /guides/userguide/implementation/
---

**Audience:** Backend engineers integrating the DeployHub API, writing GraphQL queries, or contributing to the codebase.
...
> **Implementation status:** The default `SBOMFetcher` (`internal/services.CIDFetcher`) is currently a placeholder that returns an empty component list regardless of `cid` — it does not yet fetch from IPFS or S3. See [`events/Kafka Event Schema.md`](../events/Kafka%20Event%20Schema/) for full field documentation.
...
`github_token_enc` / `gitlab_token_enc` hold encrypted PATs (see [Org-Level Personal Access Tokens](../core-concepts/architecture-guide/#org-level-personal-access-tokens)) and are only present for private repos not covered by the GitHub App installation. `is_public: true` marks an org auto-created for public-repo tracking with no credentials required.
