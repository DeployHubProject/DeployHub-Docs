...
type: guides
description: >
  What it means for a release to be "deployed" in DeployHub.
---

A **deployment**, in DeployHub' data model, is expressed as a [sync record](../sync-records/) connecting a specific [release](../releases/) to a specific [endpoint](../endpoints/) at a point in time. There's no separate "deployment" collection — the sync record itself is the deployment event.
