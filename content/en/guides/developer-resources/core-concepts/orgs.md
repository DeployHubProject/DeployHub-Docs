...
An **Org** is the top-level tenant boundary in DeployHub. Every release, endpoint, and sync record carries an `org` field, and access is scoped accordingly.
...
This GitOps RBAC model is specific to self-hosted deployments; the hosted SaaS manages orgs and roles entirely through the UI and API described in [Configure DeployHub](../../../configure-ortelius/).
