...
type: guides
description: >
  Non-functional requirements, technology stack, and deployment architecture for platform engineers and security reviewers.
aliases:
  - /guides/userguide/architecture/
---

**Audience:** Platform engineers, infrastructure teams, and security reviewers deploying or evaluating DeployHub on-premises.

For the data model itself (releases, SBOMs, CVEs, orgs, sync records), see the rest of [Core Concepts](../). This page covers non-functional requirements, technology choices, and deployment topology.
...
For the org/role model itself, see [Orgs](../orgs/).
...
This reduces database I/O from O(days × collections) to O(1 × collections). See [SLA Reference](../../../reference/sla-reference/) for the underlying targets these metrics compare against.
...
graph TB
    ...
    subgraph APICluster["API Cluster (3 replicas)"]
        API1[DeployHub Pod 1]
        API2[DeployHub Pod 2]
        API3[DeployHub Pod 3]
    end
...
## Getting a basic instance running

For the minimal steps to get a backend running against a local database, see [Self-Hosted](../../../start-here/choose-your-path/self-hosted/) in Start Here.

## Optional Integrations

### GitHub App

The GitHub App integration allows users to connect their GitHub installation and onboard repos directly from the UI. See [GitHub Integration](../../../integrations/github-integration/) for the connection flow, and configure it with:
...
Required permissions when creating the App at `https://github.com/settings/apps/new`: `Contents: Read`, `Actions: Read`. Set the callback URL to `<BASE_URL>/api/v1/auth/github/callback`.
...
Independent of the GitHub App installation flow, DeployHub supports pluggable login providers:
...
Event schema: see the [REST API Reference](../rest-api-reference/#kafka-event-schema).
...
| `SMTP_FROM_NAME`       | `DeployHub System`           | No       | From display name                                |
